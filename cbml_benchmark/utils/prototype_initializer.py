...
def initialize_prototypes_kmeans(model, cfg):
    ...
    for cls in range(cfg.LOSSES.CBML_LOSS.N_CLASSES):
        ...
        if len(feats_np) >= cfg.LOSSES.CBML_LOSS.PROTOTYPE_PER_CLASS:
            kmeans = KMeans(
                n_clusters=cfg.LOSSES.CBML_LOSS.PROTOTYPE_PER_CLASS,
                random_state=0,
                n_init=10
            ).fit(feats_np)
            centers = torch.tensor(kmeans.cluster_centers_, dtype=torch.float)
            prototypes[cls] = centers.to(cfg.MODEL.DEVICE)
            cluster_sizes[cls] = torch.tensor(np.bincount(kmeans.labels_, minlength=cfg.LOSSES.CBML_LOSS.PROTOTYPE_PER_CLASS), device=cfg.MODEL.DEVICE).float()
            del kmeans
            del centers
            torch.cuda.empty_cache()
        else:
            ...
            for k in range(cfg.LOSSES.CBML_LOSS.PROTOTYPE_PER_CLASS):
                ...
                del noise
                torch.cuda.empty_cache()
            cluster_sizes[cls] = torch.ones(cfg.LOSSES.CBML_LOSS.PROTOTYPE_PER_CLASS, device=cfg.MODEL.DEVICE) / cfg.LOSSES.CBML_LOSS.PROTOTYPE_PER_CLASS
            del mean_feat
            torch.cuda.empty_cache()
